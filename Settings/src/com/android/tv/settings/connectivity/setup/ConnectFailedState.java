...
public class ConnectFailedState implements State {
    ...
    public static class ConnectFailedFragment extends WifiConnectivityGuidedStepFragment {
            ...
            switch (status) {
                ...
                case REJECTED:
                    return getContext().getString(R.string.title_wifi_could_not_connect_ap_reject,
                            ssid);
                case TIMEOUT:
                    if ( isOtherNetworkConnected() ) {
                        return getContext().getString(R.string.title_wifi_could_not_connect_timeout_ext,ssid);
                    } else {
                        return getContext().getString(R.string.title_wifi_could_not_connect_timeout,ssid);
                    }
                default: // UNKNOWN
        ...
        private boolean isOtherNetworkConnected() {
            NetworkCapabilities networkCapabilities = null;
            Network[] networks = mConnectivityManager.getAllNetworks();

            for (Network network : networks) {
                NetworkInfo networkInfo = mConnectivityManager.getNetworkInfo(network);
                if (networkInfo.isConnected() && networkInfo.getType() == ConnectivityManager.TYPE_ETHERNET) {
                    networkCapabilities =  mConnectivityManager.getNetworkCapabilities(network);
                    break;
                }
            }
