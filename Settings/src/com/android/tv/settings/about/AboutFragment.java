...
public class AboutFragment extends SettingsPreferenceFragment {
    ...
    VnpttUntils securitySettings = null;

    SwitchPreference systemFunctionalityPref;
    ...
    public void onCreate(Bundle savedInstanceState) {
        //ChunhThanhDe
        securitySettings = VnpttUntils.getInstance(getContext());
    ...
    private int getPreferenceScreenResId() {
        switch (OverlayUtils.getFlavor(getContext())) {
            ...
            case FLAVOR_TWO_PANEL:
                VnpttUntils.LogDebug(TAG, "PreferenceScreenResId FLAVOR_TWO_PANEL");
                return R.xml.device_info_settings;
            case FLAVOR_X:
                VnpttUntils.LogDebug(TAG, "PreferenceScreenResId FLAVOR_X");
                return R.xml.device_info_settings_x;
            case FLAVOR_VENDOR:
                VnpttUntils.LogDebug(TAG, "PreferenceScreenResId FLAVOR_VENDOR");
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        //ChunhThanhDe
        systemFunctionalityPref = findPreference(KEY_SECURITY_SETTINGS);
        systemFunctionalityPref.setChecked(securitySettings.getSettingsSecurityStatus());
        systemFunctionalityPref.setOnPreferenceChangeListener(new Preference.OnPreferenceChangeListener() {
            @Override
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                ...
                if (!isEnabled) {
                    ...
                    showPasswordDialog(new PasswordDialogListener() {
                        @Override
                        public void onPasswordEntered(String password) {
                            if (checkPassword(password)) {
                                VnpttUntils.setSettingsSecurity(isEnabled);
                                Toast.makeText(getContext(), R.string.security_settings_toast_success, Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(getContext(), R.string.security_settings_toast_fail, Toast.LENGTH_SHORT).show();
                                systemFunctionalityPref.setChecked(!isEnabled);
                            }
                        }
                    });
                } else {
    ...
    public boolean onPreferenceTreeClick(Preference preference) {
        switch (preference.getKey()) {
                ...
                if (mDevHitCountdown > 0) {
                    ...
                    if (mDevHitCountdown == 0) {
                        if (securitySettings.getSettingsSecurityStatus()) {
                            // ChunhThanhDe create dialog
                            AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());

                            final EditText editText = new EditText(getActivity());
                            editText.setBackground(ContextCompat.getDrawable(getActivity(), R.drawable.vnptt_rounded_edittext_password));
                            editText.setInputType(InputType.TYPE_CLASS_NUMBER | InputType.TYPE_NUMBER_VARIATION_PASSWORD);
                            editText.setTextColor(Color.BLACK);
                            editText.setGravity(Gravity.CENTER);

                            builder.setTitle(R.string.request_development_title)
                                    .setMessage(R.string.request_development_description)
                                    .setIcon(R.drawable.vnptt_deverloper)
                                    .setCancelable(false)
                                    .setView(editText);
                            builder.setPositiveButton(R.string.request_development_ok, new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    String password = editText.getText().toString();
                                    if (password.equals(VnpttUntils.DEVELOPMENT_PASSWORD)) {
                                        ...
                                        mDevHitToast.show();
                                        mMetricsFeatureProvider.action(getContext(), MetricsEvent.ACTION_SETTINGS_BUILD_NUMBER_PREF, Pair.create(MetricsEvent.FIELD_SETTINGS_BUILD_NUMBER_DEVELOPER_MODE_ENABLED, 0));
                                    } else {
                                        Toast.makeText(getActivity(), R.string.request_development_toast_fail, Toast.LENGTH_SHORT).show();
                                        mDevHitCountdown = TAPS_TO_BE_A_DEVELOPER;
                                    }
                                }
                            });

                            builder.setNegativeButton(R.string.request_development_cancel, new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                    mDevHitCountdown = TAPS_TO_BE_A_DEVELOPER;
                                }
            ...
            case KEY_DEVICE_NAME:
                ...
                break;
            case KEY_RESET:
                VnpttUntils.LogDebug(TAG, "Action - KEY_RESET");
...
//                        "com.android.tv.settings.device.storage.ResetActivity");
                        "com.android.tv.settings.vnptt.VnpttFactoryResetActivity");
    ...
    private void showPasswordDialog(final PasswordDialogListener listener) {
        AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
        ...
        builder.setTitle(R.string.security_settings_dialog_title)
                .setMessage(R.string.security_settings_description)
                .setIcon(R.drawable.vnptt_shield_lock)
                .setCancelable(false)
                .setView(editText);
        builder.setPositiveButton(R.string.security_settings_ok, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                String password = editText.getText().toString();
        ...
        builder.setNegativeButton(R.string.security_settings_cancel, new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                systemFunctionalityPref.setChecked(securitySettings.getSettingsSecurityStatus());
                dialog.dismiss();
    ...
    private boolean checkPassword(String password) {
        return VnpttUntils.SETTINGS_PASSWORD.equals(password);
    }
