...
public class DroidSettingsModeFragment extends LeanbackPreferenceFragment implements Preference.OnPreferenceChangeListener {
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        final ListPreference NoSignalScreenStatus = (ListPreference) findPreference(NOSIGNAL_SCREEN_STATUS);
        NoSignalScreenStatus.setEntries(initNoSignalScreenStatusEntries());
        NoSignalScreenStatus.setEntryValues(initNoSignalScreenStatusEntryValue());
        NoSignalScreenStatus.setValueIndex(getNoSignalScreenStatus());
        NoSignalScreenStatus.setOnPreferenceChangeListener(this);
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        ...
        } else if (TextUtils.equals(preference.getKey(), NOSIGNAL_SCREEN_STATUS)) {
            setNoSignalScreenStatus(selection);
        }
    ...
    private boolean isAboutChannal() {
        boolean ret = false;
        int deviceId = DataProviderManager.getIntValue(getContext(),
                DroidLogicTvUtils.TV_CURRENT_DEVICE_ID, 0);
        Log.d(TAG, "isAboutChannal: deviceId is : " + deviceId);
        if (deviceId != DroidLogicTvUtils.DEVICE_ID_HDMI1
            && deviceId != DroidLogicTvUtils.DEVICE_ID_HDMI2
            && deviceId != DroidLogicTvUtils.DEVICE_ID_HDMI3
            && deviceId != DroidLogicTvUtils.DEVICE_ID_HDMI4
            && deviceId != DroidLogicTvUtils.DEVICE_ID_AV1
            && deviceId != DroidLogicTvUtils.DEVICE_ID_AV2) {
            ret = true;
        }
    ...
    private String[] initNoSignalScreenStatusEntries() {
        String[] temp = new String[2];
        if (isAboutChannal()) {
            temp[0] = "static frame";
        } else {
    ...
    private String[] initNoSignalScreenStatusEntryValue() {
        String[] temp = new String[2];
        for (int i = 0; i < 2; i++) {
            temp[i] = String.valueOf(i);
        }
    ...
    private int setNoSignalScreenStatus(int value) {
        Log.d(TAG, "setNoSignalScreenStatus: value is : " + value);
        if (value == 0) {
            mTvOptionSettingManager.setStaticFrameStatus(1, 1);
            mTvOptionSettingManager.setScreenColorForSignalChange(0, 1);
        } else {
            mTvOptionSettingManager.setStaticFrameStatus(0, 1);
            mTvOptionSettingManager.setScreenColorForSignalChange(1, 1);
        }

        return 0;
    }

    private int getNoSignalScreenStatus() {
        int staticFrameStatus = mTvOptionSettingManager.getStaticFrameStatus();
        int screenColorStatus = mTvOptionSettingManager.getScreenColorForSignalChange();
        Log.d(TAG, "getNoSignalScreenStatus: staticFrameStatus is : " + staticFrameStatus+
                   ", screenColorStatus is "+screenColorStatus);
        if (screenColorStatus == 1) {
            return 1;
        } else {
            return 0;
        }
