...
public class TvOptionSettingManager {
    ...
    public TvOptionSettingManager (Context context, boolean isothercontext) {
        ...
        mDeviceId = ((DroidLogicTvSettingsActivity)context).getIntent().getIntExtra("tv_current_device_id", -1);
        mTvSourceInput = DroidLogicTvUtils.parseTvSourceInputFromDeviceId(mDeviceId);
        mInputInfoId = ((DroidLogicTvSettingsActivity)context).getIntent().getStringExtra("current_tvinputinfo_id");
        mCurrentChannel = mTvDataBaseManager.getChannelInfo(TvContract.buildChannelUri(((DroidLogicTvSettingsActivity)context).getIntent().getLongExtra("current_channel_id", -1)));
    }
    ...
    public void setScreenColorForSignalChange(int screenColor, int isSave) {
		...
		if (screenColor == 1) {
		    mSystemControlManager.setProperty("persist.tv.blue.screen.enabled", "true");
	    } else {
			mSystemControlManager.setProperty("persist.tv.blue.screen.enabled", "false");
		}
    }
